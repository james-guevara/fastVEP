...
impl ConsequencePredictor {
    ...
    fn predict_allele(
        ...
        // Insertions use an empty Ensembl-style interval (end < start): their
        // breakpoint can be in a splice region, but they do not replace either
        // flanking essential splice base. Deletions/MNVs must test every
        // affected base, not just the first base of the allele.
        let is_insertion = var_end < var_start;
        let insertion_spans_site = |predicate: fn(&Transcript, u64) -> bool| {
            is_insertion
                && var_start > 0
                && predicate(transcript, var_start - 1)
                && predicate(transcript, var_start)
        };
        let overlaps_splice = |predicate: fn(&Transcript, u64) -> bool| {
            if is_insertion {
                insertion_spans_site(predicate)
            } else {
                (var_start..=var_end).any(|pos| predicate(transcript, pos))
            }
        };
        let overlaps_or_breakpoint = |predicate: fn(&Transcript, u64) -> bool| {
            if is_insertion {
                predicate(transcript, var_start)
            } else {
                (var_start..=var_end).any(|pos| predicate(transcript, pos))
        ...
        if overlaps_splice(splice::is_splice_donor) {
            consequences.push(Consequence::SpliceDonorVariant);
        }
        if overlaps_splice(splice::is_splice_acceptor) {
        ...
        let insertion_touches_essential_splice = is_insertion
            && var_start > 0
            && [var_start - 1, var_start].iter().any(|&pos| {
                splice::is_splice_donor(transcript, pos)
                    || splice::is_splice_acceptor(transcript, pos)
            });

        if !is_essential_splice {
            let is_donor_5th = overlaps_or_breakpoint(splice::is_splice_donor_5th_base);
            let is_donor_region = overlaps_or_breakpoint(splice::is_splice_donor_region);
            if is_donor_5th {
            ...
            if overlaps_or_breakpoint(splice::is_splice_polypyrimidine_tract) {
            ...
            if !is_donor_5th && !is_donor_region {
                if insertion_touches_essential_splice
                    || overlaps_or_breakpoint(splice::is_splice_region)
                {
        ...
        if transcript.is_coding() {
            ...
            if in_5_utr && in_exon {
                ...
                && (!is_essential_splice
                    || (!is_insertion
                        && (var_start..=var_end).any(|pos| {
                            transcript.intron_at(pos).is_some()
                                && !splice::is_splice_donor(transcript, pos)
                                && !splice::is_splice_acceptor(transcript, pos)
                        })))
            {
                // A pure essential-site allele omits intron_variant, while an
                // allele spanning both ordinary intron and the site retains it.
    ...
    fn is_in_5_utr(&self, var_start: u64, var_end: u64, transcript: &Transcript) -> bool {
        ...
        match transcript.strand {
            Strand::Forward => {
                var_start < coding_start && var_end >= transcript.start
            }
            Strand::Reverse => {
                var_end > coding_end && var_start <= transcript.end
            }
        }
    }

    fn is_in_3_utr(&self, var_start: u64, var_end: u64, transcript: &Transcript) -> bool {
        let coding_start = match transcript.coding_region_start {
            Some(s) => s,
            None => return false,
        };
...
mod tests {
    ...
    #[test]
    fn test_insertion_between_donor_bases_is_essential() {
        let transcript = make_coding_transcript();
        let predictor = ConsequencePredictor::default();
        // Intron starts at 1201. An insertion between donor bases 1201 and
        // 1202 is represented as an empty interval starting at 1202.
        let result = predictor.predict_allele(
            &GenomicPosition::new("chr1", 1202, 1201, Strand::Forward),
            &Allele::Deletion,
            &Allele::Sequence(b"T".to_vec()),
            &transcript,
            None,
        );
        assert!(result.consequences.contains(&Consequence::SpliceDonorVariant));
    }

    #[test]
    fn test_insertion_touching_one_donor_base_is_splice_region() {
        let transcript = make_coding_transcript();
        let predictor = ConsequencePredictor::default();
        // An insertion immediately before the first donor base touches the
        // essential site but does not replace or split its two bases.
        let result = predictor.predict_allele(
            &GenomicPosition::new("chr1", 1201, 1200, Strand::Forward),
            &Allele::Deletion,
            &Allele::Sequence(b"T".to_vec()),
            &transcript,
            None,
        ...
        assert!(!result.consequences.contains(&Consequence::SpliceDonorVariant));
        assert!(result.consequences.contains(&Consequence::SpliceRegionVariant));
    }

    #[test]
    fn test_deletion_spanning_acceptor_retains_intron_variant() {
        let transcript = make_coding_transcript();
        let predictor = ConsequencePredictor::default();
        // The first intron ends at 1999. This deletion covers ordinary intron
        // sequence as well as both acceptor bases 1998-1999.
        let result = predictor.predict_allele(
            &GenomicPosition::new("chr1", 1990, 1999, Strand::Forward),
            &Allele::Sequence(b"AAAAAAAAAA".to_vec()),
            &Allele::Deletion,
            &transcript,
            None,
        );
        assert!(result.consequences.contains(&Consequence::SpliceAcceptorVariant));
